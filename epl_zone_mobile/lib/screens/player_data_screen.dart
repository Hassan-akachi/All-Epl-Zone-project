...
// import '../core/network/service/player_api_service.dart';
//
...
// lib/features/player/screens/player_data_screen.dart
...
class PlayerDataScreen extends ConsumerStatefulWidget {
  const PlayerDataScreen({Key? key}) : super(key: key);

  @override
  ConsumerState<PlayerDataScreen> createState() => _PlayerDataScreenState();
}

class _PlayerDataScreenState extends ConsumerState<PlayerDataScreen> {
  final TextEditingController _searchController = TextEditingController();
            ...
            child: Column(
              ...
              children: [
                ...
                TextField(
                  ...
                  decoration: InputDecoration(
                    hintText: 'Search by name, team, nation, or position...',
                    prefixIcon: const Icon(Icons.search),
          ...
          Expanded(
            child: Card(
              ...
              child: playersAsync.when(
                data: (_) => _buildDataTable(filteredPlayers),
  ...
  Widget _buildDataTable(List<Player> players) {
    if (players.isEmpty) {
      return Center(
        child: Text(
          ref.watch(playerSearchQueryProvider).isEmpty
              ? 'No player data available'
              : 'No players found matching your search',
          style: TextStyle(
            fontSize: 16,
      ...
      child: SingleChildScrollView(
        child: DataTable(
          ...
          columns: const [
            ...
            DataColumn(label: Text('xG')),
            DataColumn(label: Text('xAG')),
            DataColumn(label: Text('Team')),
          ...
          rows: players.map((player) {
            return DataRow(
              cells: [
                ...
                DataCell(Text(player.xag?.toStringAsFixed(2) ?? '-')),
                DataCell(Text(player.team)),
                DataCell(Text(player.nationName ?? '-')),
              ],
            );
          }).toList(),
        ),
